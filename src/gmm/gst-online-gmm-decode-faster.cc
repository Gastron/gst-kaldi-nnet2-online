...
// The original contributions of Ricard Marxer copied from gstkaldinnet2onlinedecoder to this plugin by:
...
static void gst_online_gmm_decode_faster_class_init(GstOnlineGmmDecodeFasterClass * klass) {
  ...
  g_object_class_install_property(G_OBJECT_CLASS(klass),
                                  PROP_PHONE_SYMS,
                                  g_param_spec_string("phone-syms",
                                                      "Phone symbols",
                                                      "Name of phone symbols file (typically phones.txt)",
                                                      DEFAULT_PHONE_SYMS,
                                                      (GParamFlags) G_PARAM_READWRITE));
  g_object_class_install_property(G_OBJECT_CLASS(klass),
                                  PROP_DO_PHONE_ALIGNMENT,
                                  g_param_spec_boolean("do-phone-alignment",
                                                       "Phone-level alignment",
                                                       "If true, output phone-level alignment",
                                                       FALSE,
                                                       (GParamFlags) G_PARAM_READWRITE));
  ...
  gst_online_gmm_decode_faster_signals[HYP_WORD_SIGNAL]
      = g_signal_new("hyp-word", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                     ...
                     G_TYPE_STRING);
  gst_online_gmm_decode_faster_signals[HYP_PHONE_ALIGNMENT_SIGNAL]
      = g_signal_new("hyp-phone-alignment", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                    G_STRUCT_OFFSET(GstOnlineGmmDecodeFasterClass, hyp_phone_alignment),
                    NULL, NULL, kaldi_marshal_VOID__STRING, G_TYPE_NONE, 1,
                    G_TYPE_STRING);
}
...
gst_online_gmm_decode_faster_init(GstOnlineGmmDecodeFaster * filter) {
  ...
  filter->phone_syms_filename_ = g_strdup(DEFAULT_PHONE_SYMS);
  ...
  filter->do_phone_alignment_ = false;
...
gst_online_gmm_decode_faster_allocate(GstOnlineGmmDecodeFaster * filter) {
  if (!filter->decoder_) {
    ...
    if (!filter->phone_syms_) {
      if (strcmp(filter->phone_syms_filename_, "") != 0) {
        if (!(filter->phone_syms_ = fst::SymbolTable::ReadText(filter->phone_syms_filename_))) {
          GST_ERROR_OBJECT(filter, "Could not read symbol table from file %s", 
              filter->phone_syms_filename_);
          return false;
        }
      }
    }
...
gst_online_gmm_decode_faster_finalize(GObject * object) {
  ...
  g_free(filter->phone_syms_filename_);
...
gst_online_gmm_decode_faster_set_property(GObject * object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_PHONE_SYMS:
      g_free(filter->phone_syms_filename_);
      filter->phone_syms_filename_ = g_value_dup_string(value);
      break;
    case PROP_DO_PHONE_ALIGNMENT:
      filter->do_phone_alignment_ = g_value_get_boolean(value);
      break;
    case PROP_LDA_MAT:
      g_free(filter->lda_mat_rspecifier_);
...
gst_online_gmm_decode_faster_get_property(GObject * object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_WORD_SYMS:
      ...
      break;
    case PROP_PHONE_SYMS:
      g_value_set_string(value, filter->phone_syms_filename_);
      break;
    case PROP_DO_PHONE_ALIGNMENT:
      g_value_set_boolean(value, filter->do_phone_alignment_);
      break;
    case PROP_LDA_MAT:
 ...
 */
static void
gst_online_gmm_decode_faster_push_word(GstOnlineGmmDecodeFaster * filter, GstPad *pad, std::string word, bool newline) {
  ...
  if (newline) {
    gst_buffer_memset(buffer, hyp_len, "\n", 1);
  } else {
...
gst_online_gmm_decode_faster_push_words(GstOnlineGmmDecodeFaster * filter, GstPad *pad,
  ...
  for (size_t i = 0; i < words.size(); i++) {
    ...
    gst_online_gmm_decode_faster_push_word(filter, pad, word, false);
  }
  gst_online_gmm_decode_faster_push_word(filter, pad, "", true);
}

static void 
gst_online_gmm_decode_faster_phone_align(
  GstOnlineGmmDecodeFaster * filter, 
  const std::vector<int32>& alignment,
  BaseFloat frame_shift_in_sec
  ) {
  if (strcmp(filter->phone_syms_filename_, "") == 0) {
    GST_ERROR_OBJECT(filter, "Phone symbol table filename (phone-syms) must be set to do phone alignment.");
  } else if (filter->phone_syms_ == NULL) {
    GST_ERROR_OBJECT(filter, "Phone symbol table was not loaded correctly. Not doing phone alignment.");
  } else {
    GST_DEBUG_OBJECT(filter, "Doing phone alignment...");
    std::vector<std::vector<int32> > split;
    SplitToPhones(*filter->trans_model_, alignment, &split);
    ...
    for (size_t i = 0; i < split.size(); i++) {
      ...
      int32 phone = filter->trans_model_->TransitionIdToPhone(split[i][0]);
      std::string s = filter->phone_syms_->Find(phone);
      if (s == "") {
        GST_ERROR_OBJECT(filter, "Phone-id %d not in symbol table", phone);
      }
      int32 num_repeats = split[i].size();
      phone_alignment << s << " " << num_repeats*frame_shift_in_sec << std::endl;
    }
    ...
    gst_pad_push(filter->srcpad_, pali_buffer);
    /*Emit signal*/
    g_signal_emit(filter, 
        gst_online_gmm_decode_faster_signals[HYP_PHONE_ALIGNMENT_SIGNAL], 
        0, phone_alignment.str().c_str());
  }
...
gst_online_gmm_decode_faster_loop(GstOnlineGmmDecodeFaster * filter) {
  ...
  BaseFloat frame_shift_in_sec = frame_shift / 1000.0; //convert ms to s
  ...
  while (1) {
    ...
    if (dstate & (filter->decoder_->kEndFeats | filter->decoder_->kEndUtt)) {
      ...
      partial_res = false;
      if (word_ids.size()>0 && filter->do_phone_alignment_) {
        gst_online_gmm_decode_faster_phone_align(filter, alignment, frame_shift_in_sec);
      }
